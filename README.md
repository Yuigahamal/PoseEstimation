...
注意这里**没有任何归一化操作**。即去掉了前面的归一化系数$\frac{1}{2 \pi \sigma ^2}$
...
为了缓解 RNN 的问题,人们提出了一 些基于姿势跟踪的方法,为视频帧中的每个人建立一个 轨迹,以过滤不相关信息的干扰。
...
## 3. 后处理方法
些方法不是立即预测最终关键点位置,而是首先估计初始姿态,然后通过一些后处理操作对其进行优化, 我们将其称为后处理方法。我们将这些方法分为两类, 即量化误差和姿态重采样。

### 3.1 量化误差

**量化误差**指的是：**真实的连续物理坐标被强制映射到离散的像素网格上时所产生的精度损失。**
假设有一张$64*64$的热图，对应原图$256*256$的区域。这意味着热图上的1个像素对应原图上的4个像素。
若某人的手腕关节真实位置在原图的$(100.5,200.7)$处。这是一个非常精确的连续坐标。
为了生成热图或从热图解码，这个坐标必须被转换到热图的整数索引上：
$$
热图x = round(100.5 / 4) = 25
$$
$$
热图y = round(200.7 / 4) = 50
$$
因此系统认为关节在热图的$(25,50)$位置
当从热图取最大值点$(25,50)$并乘以下采样倍数还原时:
$$
预测x = 25 * 4 = 100
$$
$$
...
Y轴误差 = 200.7 - 200 = 0.7
$$
这0.5 和 0.7 像素的偏差 就是量化误差。无论你的模型特征提取多么完美，只要最后一步是“取热图最大激活值的整数索引”，这个误差就不可避免。
低分辨率放大误差： 为了节省显存，热图通常只有原图的$1/4$甚至$1/8$大小。热图上差1个像素，还原到原图就差4~8个像素。对于手部、面部等精细关键点，几个像素的误差可能意味着完全定位错误。
对于关键点的热图表示, 从热图到坐标空间的转换不可避免地会出现错误,从而导致量化误差。抑制此类量化误差将提高许多基于热图的模型的性能。
而且我们希望预测出来的热力图是符合标准二维高斯分布，**但实际得到的热图并不是总是符合标准高斯分布，并且有可能存在多个峰值**，这降低了坐标计算的准确性。

#### 3.1.1 distribution-aware architecture（分布感知架构）

原论文为 Distribution-Aware Coordinate Representation for Human Pose Estimation (DARK)
分布感知架构，其核心思想可以概括为：**“先整形，再定位”**。
传统的基于热图的方法隐含了一个强假设：**预测出的热图是一个标准的、单峰的各向同性二维高斯分布**。但在实际训练中，由于遮挡、复杂背景或网络感受野的限制，预测出的热图往往是：

- 扁平的： 峰值不明显，最大值区域是一片平坦的区域。
- 多峰的： 出现多个局部极大值（例如把肘部和手腕混淆）。
- 不对称/拖尾的： 形状像逗号或哑铃，而非圆形。
当你在一个“扁平”或“多峰”的热图上直接取 argmax 时，哪怕只偏离了1个像素，还原到原图就是巨大的量化误差。**分布感知架构认为：与其改进解码公式，不如先把热图“修”成标准高斯形状，让 argmax 自然变得准确。**

分布感知架构通常包含两个关键模块:
- 热图分布调制：引入一个轻量级的子网络或注意力机制，对原始预测热图进行自适应调整。它会抑制杂散的副峰、增强主峰、并将扁平的区域“捏”成尖锐的单峰。
- 分布感知解码： 不再盲目信任全局最大值，而是结合热图的分布统计特性（如方差、置信度）来计算坐标。例如，只在调制后的高置信度区域内进行亚像素级插值。

我们假设一个要定位一个人右膝关键点，热图分辨率为$64*64$,对应原图$256*256$
若使用传统的热力图方法，则可能模型预测出的右膝热图是扁平且略微双峰的：
- 真实关节位置在热图坐标$(30.6,45.8)$
- 但由于热图扁平，最大激活值出现在$(31,45)$,而次大值在$(30,46)$,两者激活值紧差0.01
- `argmax`取到$(31,45)$
- 还原坐标: $(31* 4,45*4) = (124,180)$
- 真实坐标: $(30.6*4,45.8*4) = (122.2,183.4)$
- 误差: $\sqrt{(124-122.2)^2 + (180-183.4)^2} = 3.6$像素

当采用分布感知架构处理时，
- 调制阶段： 分布调制模块识别出该热图是“扁平双峰”的异常形态，自动对其进行锐化处理，抑制$(30,46)$处的副峰，将主峰集中在$(30.6,45.8)$附近，使其变成尖锐的单峰高斯。
- 解码阶段: 在调制后的尖锐热图上，最大值现在稳定地落在$(31,46)$甚至通过亚像素插值得到$(30.6,45.8)$


计算过程如下:
目的： 解决热图最大值点偏离真实峰值的问题。
具体做法： 对预测热图H在最大激活点m处进行二阶泰勒展开，利用梯度$\nabla H(m)$和海森矩阵$\nabla^2 H(m)$来估计真实峰值的偏移量
计算步骤:
1. 找到热图全局最大值的整数坐标$m=(x_m,y_m)$
2. 计算该点的梯度向量$g= \nabla H(m)$和海森矩阵$H_e = \nabla ^2 H(m)$
3. 计算偏移量:$\Delta m = -{H_e}^{-1} g$
4. 得到调制后的亚像素坐标: $m ^* = m + \Delta m$

假设热图分辨率为$64*64$，我们关注右膝关键点
- 步骤1：找最大值
  预测热图$H$ 在整数点$m=(30,45)$处取得最大值$H(30,45)=0.95$
- 步骤 2：计算梯度和海森矩阵（有限差分）
  x方向梯度:
  $$
  g_x \approx \frac{H(31,45) - H(29,45)}{2}= \frac{0.92 - 0.94}{2} = -0.01
  $$
  y方向梯度  
  $$
  g_y \approx \frac{H(30,46) - H(30,44)}{2}= \frac{0.93 - 0.96}{2} = -0.015
  $$
  梯度矩阵g
  $$
  g = \begin{bmatrix} g_x \\ g_y \end{bmatrix} = \begin{bmatrix} -0.01 \\ -0.015 \end{bmatrix}
  $$
  海森矩阵:
  $$
  h_{xx} \approx H(31,45) + H(29,45) - 2*H(30,45) = 0.92 + 0.94 - 2*0.95 = -0.04
  $$
  $$
  h_{yy} \approx H(30,46) + H(30,44) - 2*H(30,45) = 0.93 + 0.96 - 2*0.95 = -0.01
  $$
  为了简化，假设非对角元$h_{xy} \approx 0$
  则海森矩阵为
  $$
  H_{e} = \nabla^2 H(m) = \begin{bmatrix} h_{xx} & h_{xy} \\ h_{xy} & h_{yy} \end{bmatrix} = \begin{bmatrix} -0.04 & 0 \\ 0 & -0.01 \end{bmatrix}
  $$
- 步骤3: 计算偏移量
  $$
  \Delta m = -{H_e}^{-1} g = - \frac{1}{h_{xx}h_{yy} - h_{xy}^2} \begin{bmatrix} h_{yy} & -h_{xy} \\ -h_{xy} & h_{xx} \end{bmatrix} \begin{bmatrix} g_x \\ g_y \end{bmatrix} = \begin{bmatrix} -0.25 \\ -1.5 \end{bmatrix}
  $$
  $$
  \Delta x = -0.25     
  $$
  ...
  \Delta y = -1.5
  $$
  注意：$\Delta y= -1.5$超出了合理范围(通常应$ <0.5 $)。这说明热图在 y 方向严重偏离高斯假设。DARK 论文中会对此做截断处理：若$|\Delta|> 0.5 $，则裁剪至$0.5$或回退到原始argmax。假设裁剪后$\Delta y = -0.5$。
- 步骤4: 得到调制后的亚像素坐标并还原到原图（假设4倍下采样）
  $$
  m ^* = m + \Delta m = (30,45) + (-0.25,-0.5) = (29.75,44.5)
  $$
  $$
  p^* = m ^* * 4 = (29.75*4,44.5*4) = (119.0,178.0)
  $$
#### 3.1.2 无偏数据处理与理论无差错转换 
论文The Devil is in the Details: Delving into Unbiased Data Processing for Human Pose Estimation (DUP)
传统的热力图如果使用像素个数来衡量距离和位置，那始终无法避免**量化误差**的问题，这篇论文 提出改用归一化的单位长度（即坐标占图像宽/高的比例，范围 0~1）来处理所有数据。

### 3.2 姿态重采样
大多数姿态估计方法（如 SimpleBaseline、HRNet 等）的工作方式是：模型输出热图 → 取 argmax 得到坐标 → 直接作为最终结果，不再做任何后处理。
然而，这些估计可以通过与模型无关的姿态重采样技术进一步改进。
#### 3.3 Posefix
Posefix: Modelagnostic general human pose refinement network 提出了一种提出了一种与模型无关的 PoseFix 方法
该方法根据输入图像和输入姿势的元组 估计精细姿势,其中输入姿势是从现有方法的估计中得出的。
PoseFix 的做法是：
1. 先用任意现有方法（比如 HRNet）得到一个粗糙的姿态（17个关键点坐标）。
2. 把这个粗糙姿态 + 原始图像一起喂给 PoseFix 网络。
3. PoseFix 输出一个更精细的姿态。

#### 3.4 GCN 模块估计不可见关节
这篇工作专门解决被遮挡关节的问题。分两步：
第一步： 用现有方法（HRNet）先定位那些看得见的关节（比如手臂没被挡住，肘部可见）。
第二步： 对于那些被遮挡、看不见的关节（比如被身体挡住的另一只手），用一个图卷积网络（GCN）来推断。GCN 的输入有两个：
- 图像上下文： 原图中被遮挡区域周围的视觉信息（比如虽然手被挡住了，但能看到肩膀的位置）。
- 姿势结构线索： 人体骨骼的先验知识（比如左右肩宽度大致相等，知道左肩位置就能推断右肩位置）。
类比理解：你看到一个人的左半身，右半身被墙挡住了。你根据"人体左右大致对称"这个常识，推测出右半身的位置。
#### 3.5 Graph-PCNN
和 PoseFix 思路类似，也是"先粗后精"的两阶段：
粗定位： 用任意现有方法得到初始关键点。
精细化： 用一个图姿态细化模块（基于图结构建模关节间关系）来修正初始结果。
与 Posefix 的区别：Posefix 重点在补全缺失的关节，而 Grpah-PCNN 重点在精修所有关节的位置精度。
